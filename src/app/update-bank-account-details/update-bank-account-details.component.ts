...
@Component({
  selector: 'app-update-bank-account-details',
  templateUrl: './update-bank-account-details.component.html',
  styleUrls: ['./update-bank-account-details.component.sass']
})
export class UpdateBankAccountDetailsComponent implements OnInit {
  ifscForm: FormGroup;
  ...
  constructor(private formBuilder: FormBuilder, private cmn: CommonComponent, 
    private service: UpdateBankAccountDetailsService,private fb: FormBuilder,
    private http: Http, private router: Router, private route: ActivatedRoute) {
    $('.page-loader-wrapper').hide();

    var json_response;
    json_response = eval('(' + sessionStorage.getItem('update_details') + ')');

    console.log(json_response);

    this.finproaccId = json_response.finproaccId;
    this.finsiteAccMapId = json_response.finsiteAccMapId;
    this.adressId = json_response.adressId;


    $(function () {
      $("#Account_Holder_Name").val(json_response.holder);
      $("#Enter_Account_Number").val(json_response.accountno);
      $("#Enter_Branch_Name").val(json_response.branch);
      $("#Enter_ISFC_Code").val(json_response.ifsc);
      $("#address_data").val(json_response.adress);
    });

    if (json_response.finBankId != null && json_response.finBankId != "") {
      this.bank_list_name = [
        {
          id: json_response.finBankId,
          bankName: json_response.bankName
        }
      ]

      this.controller_bank_data = json_response.finBankId;
    }

    if (json_response.siteId != null && json_response.siteId != "") {
      this.project_list_name = [
        {
          id: json_response.siteId,
          name: json_response.siteName
        }
      ]

      this.controller_site_data = json_response.siteId;
    }

    this.owner_type_list(json_response.siteId);

    if (json_response.setofftypr != null && json_response.setofftypr != "") {
      this.setoff_list_name = [
        {
          id: json_response.setofftypr,
          key: json_response.setofftype
        }
      ]

      this.controller_setoff_data = json_response.setofftypr;

    }


    setTimeout(() => {
      if (json_response.saleOwnerId != null && json_response.saleOwnerId != "") {
        this.owner_list_name = [
          {
            id: json_response.saleOwnerId,
            flatSaleOwner: json_response.saleOwner
          }
        ]

        this.controller_owner_data = json_response.saleOwnerId;
  ...
  update_function() {
    ...
    if (confirm("Are you sure you want to update the account details ?")) {
      $('.page-loader-wrapper').show();
      this.service.Get_save_function(this.Enter_Account_Number, this.Enter_Branch_Name,
        this.address_data, this.controller_bank_data, this.controller_site_data,
        this.controller_owner_data, this.controller_setoff_data, this.Account_Holder_Name,
        this.Enter_ISFC_Code, this.finproaccId, this.finsiteAccMapId, this.adressId).then(resp => {
          console.log(resp);
          $('.page-loader-wrapper').hide();
          if (resp.responseCode == 200) {
            swal(resp.status);
            this.router.navigateByUrl("View_Bank_Account_Details");
            return false;
  ...
  add_account_fun() {
    this.router.navigate(['Add_Bank_Account_Details']);
  }

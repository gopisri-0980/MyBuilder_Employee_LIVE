...
import { AssignmentCostDetailsReuestModule } from '../model/assignment-cost-details-reuest.module';
...
@Component({
  selector: 'app-assignment-transfer-free',
  templateUrl: './assignment-transfer-free.component.html',
  styleUrls: ['./assignment-transfer-free.component.sass']
})
export class AssignmentTransferFreeComponent implements OnInit {
  ...
  modificationCostDetailsRequest: AssignmentCostDetailsReuestModule;
  isValidFormSubmitted = false;
  modiCostDtlsRequests: Array<AssignmentCostDetailsReuestModule> = [];
    ...
    $(function () {
      ...
      $('#flatSelection').change(function (e) {
        ...
        if (selected_flatid == "select") {
          ...
          // console.log(self.milestoneArray.length)
          // if(self.milestoneArray.length >= 1){
          //   for(var i=0;i<self.milestoneArray.length;i++){
  ...
  getGSTPercentage() {
    ...
    var body = {
      "sessionKey": "" + sessionStorage.getItem("login_sessionkey"),
      "typeName": "ASSIGNMENT_TRANSFER_FEE",
      "siteIds": [$("#projectID").val()],
    }
  ...
  convertMilestonerArrayToModificationCostDetailsRequestArray() {
    ...
    for (var milestone of this.milestoneArray) {
      ...
      this.modificationCostDetailsRequest = {
        assignmentTransferFeeChargeDesc: milestone.detailsOfModification,
  ...
  onSubmit(form) {
    ...
    let url = this.cmn.commonUrl + "financial/doAssignmentTransferFeeEntry.spring"
    ...
    console.log(resp);
      ...
      if (resp.responseCode == 200) {
       ...
       setTimeout(() => {
        ...
        swal("Assignment Transfer Fee Invoice  sent Successfully !!");
        return false;
  ...
  percentagesList(siteid) {
    ...
    //https://106.51.38.64:7777/SumadhuraGateway/employeeservice/financial/mileStonePercentages.spring
    let headers = new Headers({ 'Content-Type': 'application/json' });
    var body = {
      "sessionKey": "" + sessionStorage.getItem("login_sessionkey"),
      // "typeName": "ASSIGNMENT_TRANSFER_FEE",

...
var old_flatbooking_id_search;
var old_booking_customer_id;

@Component({
  selector: 'app-existing-booking',
  templateUrl: './existing-booking.component.html',
  styleUrls: ['./existing-booking.component.sass']
})
export class ExistingBookingComponent implements OnInit {
  ...
  mileStoneAliasName: any;
  close_button_refresh: any;
  financialProjectMileStone: Array<any> = [];

  constructor(public service: ExistingBookingService, private cmn: CommonComponent, private http: Http, private formBuilder: FormBuilder,
    private router: Router, private modalService: NgbModal, private changeDetectorRef: ChangeDetectorRef, private sanitizer: DomSanitizer, public snackBar: MatSnackBar) {
  ...
  ngOnInit() {
    ...
    $(function () {
      ...
      $("#sales_fores_booking_Id").select2({
        placeholder: "--Select--",
        dir: "ltl",
      });
    ...
    $(function () {
      ...
      $('#sales_fores_booking_Id').change(function (e) {

        var booking_id_val = $(e.target).val();

        old_flatbooking_id_search = booking_id_val.split("&&")[0];
        old_booking_customer_id = booking_id_val.split("&&")[1];
        old_flatbooking_id = booking_id_val.split("&&")[2];
        old_flatbooking_name = booking_id_val.split("&&")[2];



        for (var i = 0; i < self.main_div_controller.length; i++) {



          if (self.main_div_controller[i].salesforceBookingId == old_flatbooking_id) {

            if (self.main_div_controller[i].statusId == 56) {


              $("#unit_project").prop('disabled', false);
              $("#unit_block").prop('disabled', false);
              $("#unit_floor").prop('disabled', false);
              $("#unitno").prop('disabled', false);

              $("#unit_project").css({ backgroundColor: '#eee' });
              $("#unit_block").css({ backgroundColor: '#eee' });
              $("#unit_floor").css({ backgroundColor: '#eee' });

              $("#unitno").css({ backgroundColor: '#FFF' });
            } else {



              $("#unit_project").prop('disabled', true);
              $("#unit_block").prop('disabled', true);
              $("#unit_floor").prop('disabled', true);
              $("#unitno").prop('disabled', true);

              $("#unit_project").css({ backgroundColor: '#FFF' });
              $("#unit_block").css({ backgroundColor: '#FFF' });
              $("#unit_floor").css({ backgroundColor: '#FFF' });
              $("#unitno").css({ backgroundColor: '#eee' });
      ...
      $('#search_projectID').change(function (e) {
        ...
        if (search_projectid == "select") {
          ...
          self.get_oldsalesforesbg_old(search_projectid);
  ...
  get_oldsalesforesbg_old(selectedSiteID) {
    $('.page-loader-wrapper').show();
    this.service.get_oldsalesforesbookingid(selectedSiteID).then(resp => {

      this.main_div_controller = resp.responseObjList;


      $('.page-loader-wrapper').hide();
      if (resp.responseCode == 200) {
        $('#sales_fores_booking_Id').html("");
        $('#sales_fores_booking_Id').append('<option value="select">--Select--</option>');
        for (var i = 0; i < resp.responseObjList.length; i++) {
          $('#sales_fores_booking_Id').append("<option value='" + resp.responseObjList[i].flatBookingId + "&&" + resp.responseObjList[i].customerId + "&&" + resp.responseObjList[i].salesforceBookingId + "'>" + resp.responseObjList[i].bookingIdWithFlatNoAndStatus + "</option>");
        }
      } else if (resp.responseCode == 440) {
        swal("Your Session has been Timed Out!", "Please login once again.", "error");
  ...
  search_function() {
    ...
    if (old_flatbooking_id_search == "select" || old_flatbooking_id_search == undefined) {
      swal("Please select the sales fores booking id!");
      return false;
    }
    ...
    $('.page-loader-wrapper').show();
    var flatBookingId = old_flatbooking_id_search;
    var cutID = old_booking_customer_id;
  ...
  getBookingFormDetails(userdata) {
    ...
    this.get_site_wise_list(unitDetails.siteInfo.siteId);
  ...
  total_costfun(event) {
    ...
    if ($("#sbua_unit").val() != "" && $("#unit_total_cost_exclude_gst").val()) {
      const ratepersqrt = parseFloat($("#unit_total_cost_exclude_gst").val());
      const taxespersft = parseFloat($("#sbua_unit").val());
      const basicsum = (ratepersqrt / taxespersft).toFixed(2);
  ...
  sbua_aminititesInfranamefun(event) {
    ...
    if ($("#unit_total_cost_exclude_gst").val() != "") {
      const num1 = parseFloat($("#unit_total_cost_exclude_gst").val());
      const num2 = parseFloat($("#Scheme_name").val());
      const sum = (num1 * num2 / 100).toFixed(2);
      $("#unit_total_gst_amount").val(sum);
    }
  ...
  ratepersqft_keyFunc(event) {
    ...
    console.log($("#unit_total_cost_exclude_gst").val());
  ...
  Scheme_Name(siteids) {
    ...
    this.service.get_schemes_list(siteids).then(resp => {
      ...
      if (resp.responseCode == 200) {
        ...
        console.log(bookingForData.customerSchemeInfos[0].percentageValue);

        if (bookingForData.customerSchemeInfos[0].schemeName == "GST 5% Booking") {
  ...
  /*-----------------Getting Project(site) list Start---------------------*/
  siteList() {
    var arr = localStorage.getItem('Create_Booking_With_Existing_Data');
  ...
  get_oldsalesforesbg(selectedSiteID) {
    ...
    this.service.get_oldsalesforesbookingid(selectedSiteID).then(resp => {
      ...
      if (resp.responseCode == 200) {
        ...
        if (bookingForData.flatBookingInfo.OldBookingName != undefined && bookingForData.flatBookingInfo.OldBookingName != null && bookingForData.flatBookingInfo.OldBookingName != "select" && bookingForData.flatBookingInfo.OldBookingName != "--Select--") {
          $('#unit_oldbooking_name').val(bookingForData.flatBookingInfo.OldBookingName);
        } else {
          $('#unit_oldbooking_name').val(bookingForData.flatBookingInfo.BookingId);
        }
        old_flatbooking_name = bookingForData.flatBookingInfo.OldBookingName;

        old_flatbooking_id = bookingForData.flatBookingInfo.BookingId;
    ...
    this.service.getunit_details(projectid, blockid, floorDetId).then(resp => {
      ...
      if (resp.responseCode == 200) {
        ...
        if (unitDetails.flatInfo.flatId != null && unitDetails.flatInfo.flatId != "" && unitDetails.flatInfo.flatId != undefined) {
          ...
          $("#unitno").val(unitDetails.flatInfo.flatId);
  ...
  submitfun() {
    ...
    //     if (this.kyc_controller[i].files != null) {
    //       if (this.kyc_controller[i].files.length == 0 && this.kyc_controller[i].status != "true") {
    ...
    if ($("#unit_amenitiesFlatCost").val() != "" && $("#unit_basic_flat_cost").val() != "") {
      const aminitycost = parseFloat($("#unit_amenitiesFlatCost").val());
      const basicflatcost = parseFloat($("#unit_basic_flat_cost").val());
      const sum = (aminitycost + basicflatcost).toFixed(2);
      const discount = $("#unit_discount").val();
      if (parseFloat(sum) < parseFloat(discount)) {
        swal("Discount amount is more than total flat cost");
        return false;
      }
    ...
    if ($("#unit_amenitiesFlatCost").val() != "" && $("#unit_basic_flat_cost").val() != "") {
      ...
      const basicflatcost = parseFloat($("#unit_basic_flat_cost").val());
      const sum = (aminitycost + basicflatcost).toFixed(2);
      const discount = $("#unit_AAA").val();
      if (parseFloat(sum) < parseFloat(discount)) {
        swal("AAA amount is more than total flat cost");
        return false;
      }
    ...
    var flatBookingInfo = {
      ...
      "OldBookingName": old_flatbooking_id,
    ...
    this.financialProjectMileStone = [];
  ...
  tabs_function_event(event) {
    ...
    } else if (event == 'SECOND APPLICANT ') {
  ...
  milestoneDetails() {
    this.mileStoneAliasName = bookingForData.employeeFinancialRequest.mileStoneAliasName;
    console.log(this.mileStoneAliasName);
    console.log(bookingForData);

  }


  milestone_view_fun(content) {
    $('.page-loader-wrapper').show();
  ...
  get_site_wise_list(site_list) {
    ...
    this.service.get_site_list_details(site_list).then(resp => {
      ...
      if (resp.responseCode == 200) {
        ...
        console.log(bookingForData.employeeFinancialRequest.finMilestoneClassifidesId);

        if (bookingForData.employeeFinancialRequest.finMilestoneClassifidesId != null && bookingForData.employeeFinancialRequest.finMilestoneClassifidesId != undefined) {
          $("#Select_Alias_Name").val(bookingForData.employeeFinancialRequest.finMilestoneClassifidesId);
        }

        console.log(this.view_milestone_cont);
        if (this.view_milestone_cont !== undefined) {

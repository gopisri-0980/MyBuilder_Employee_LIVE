...
export class AssignTicketComponent implements OnInit {
  ...
  handleChange(event, data) {
    ...
    $('.page-loader-wrapper').show();
    let url = this.cmn.commonUrl + "employeeTicket/getCrmEmployees.spring";
    let headers = new Headers({ 'Content-Type': 'application/json' });
    var body = {
      "sessionKey": "" + sessionStorage.getItem("login_sessionkey"),
  ...
  AddtoBoxfun() {
    ...
    for (var i = 0; i < this.controller.length; i++) {
      console.log(this.controller[i].ticketType);
      console.log(this.title_value);
      if( this.controller[i].ticketType == this.title_value && this.controller[i].siteId == $("#projectID").val() && JSON.stringify(this.controller[i].blockIds) === JSON.stringify($("#BlockId").val()) ){
        swal("Given ticket type already configured with employee!");
        return false;
      }
     }
  ...
  submitfunction() {
    ...
    console.log(JSON.stringify($("#BlockId").val()));
      ...
      for (var i = 0; i < this.controller.length; i++) {
        ...
        if( this.controller[i].ticketType == this.title_value && this.controller[i].siteId == $("#projectID").val() && JSON.stringify(this.controller[i].blockIds) === JSON.stringify($("#BlockId").val()) ){
          ...
          this.controller_data.push({
            ...
            "ticketType": this.controller[i].ticketType,
          });
        }
      ...
      if (confirm("Do you really want to Submit ?")) {
        ...
        $('.page-loader-wrapper').show();
        //http://localhost:8888/SumadhuraGateway/employeeservice/employeeTicket/createTicketTypeDetailsForCRM.spring
        let url = this.cmn.commonUrl + "employeeTicket/createTicketTypeDetailsForCRM.spring";
        let headers = new Headers({ 'Content-Type': 'application/json' });
        var body = {
          "sessionKey": "" + sessionStorage.getItem("login_sessionkey"),
          "ticketTypeRequestList": this.controller_data,
        }
      ...
      // swal("Please complete incomplete ticket ?");
      // return false;

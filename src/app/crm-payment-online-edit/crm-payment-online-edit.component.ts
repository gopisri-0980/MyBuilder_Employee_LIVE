...
@Component({
  selector: 'app-crm-payment-online-edit',
  templateUrl: './crm-payment-online-edit.component.html',
  styleUrls: ['./crm-payment-online-edit.component.css']
})
export class CrmPaymentOnlineEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    } else if (this.operationType == "loadCompletedTransaction" || this.operationType == "ViewCustomerData") {
      //alert("hello")
      this.Edit_hide_show = true;
      //$('#FaltId').prop("disabled", false);
  ...
  editSubmit() {
    ...
    if ($("#ChequeNumber").val() == "") {
      swal("Please enter reference number");
      $('#ChequeNumber').focus();
      return false;
    }
    ...
    if ($("#PrincipalAmount").val() == "0" && $("#ModificationCharges").val() == "0" && $("#InterstAmount").val() == "0" && $("#LeagalCharges").val() == "0" && $("#refunableCharges").val() == "0" && this.Registration_And_Mutation_Charges == "0" && this.Non_refundable_Caution_Deposit == "0" && this.Electricity_Deposit == "0") {
    ...
    if ((Number($('#PrincipalAmount').val()) + Number($('#ModificationCharges').val()) + Number($('#InterstAmount').val()) + Number($('#LeagalCharges').val()) + Number($('#refunableCharges').val()) + Number(this.Registration_And_Mutation_Charges) + Number(this.Non_refundable_Caution_Deposit) + Number(this.Electricity_Deposit)) > Number($('#ChequeAmount').val())) {
    ...
    if (confirm("Do you want to Submit the Page ?")) {
      ...
      var body = {
        ...
        "paymentSetOffDetails": [
          ...
          , {
            "setOffTypeName": "Registration_And_Mutation_Charges", "amount": this.Registration_And_Mutation_Charges, "invoiceNo": null,
            "refundFromMileStone": this.refundForMilestone
          }
          , {
            "setOffTypeName": "Non_refundable_Caution_Deposit", "amount": this.Non_refundable_Caution_Deposit, "invoiceNo": null,
            "refundFromMileStone": this.refundForMilestone
          }
          , {
            "setOffTypeName": "Electricity_Deposit", "amount": this.Electricity_Deposit, "invoiceNo": null,
            "refundFromMileStone": this.refundForMilestone
          }
  ...
  modification_legalInvoiceList(flatBookingId) {
    ...
    var body = {
      ...
      "sessionKey": "" + sessionStorage.getItem("login_sessionkey"),
      "bookingFormId": Number(flatBookingId), // 276
      "requestUrl": "CompletedTransaction",
      "transactionTypeId": $("#transaction_type").val(),
      "transactionModeId": "2",
      "transferModeId": "1",
  ...
  bindingDetails() {
    ...
    this.http.post(url, body, options).map(res => res.json()).subscribe(resp => {
      ...
      if (resp.responseCode == 200) {
        this.resp_onse = resp;
        $("#ChequeNumber").val(resp.responseObjList.finTransactionEntryDetailsResponseList[0].referenceNo)
        this.chequeDate = resp.responseObjList.finTransactionEntryDetailsResponseList[0].transactionDate

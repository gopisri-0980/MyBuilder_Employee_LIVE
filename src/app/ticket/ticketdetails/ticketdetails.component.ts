...
@Component({
  ...
  styleUrls: ['./ticketdetails.component.sass']
})
export class TicketdetailsComponent implements OnInit {
  ...
  constructor(private _router: Router, private deviceService: DeviceDetectorService,
    ...
    if (sessionStorage.getItem("common_for_flats") == "view_closed_tickets") {
      ...
      console.log(this.jsontotalticketresponse);
      ...
      for (var i = 0; i < this.jsontotalticketresponse.length; i++) {
        if (this.jsontotalticketresponse[i].ticketId == this.ticketdetailsresponse.ticketId) {
          this.pageIndex = i;
        }
      }
    }
    ...
    } else if (sessionStorage.getItem("common_for_flats") == "viewtickets_graph") {
      this.binding_parentpage = "Ticket Graph";
      // alert(this.binding_parentpage);
    ...
    } else if (sessionStorage.getItem("common_for_flats") == "view_my_complaints") {
      this.binding_parentpage = "View My Complaints";
    }
    ...
    // alert(this.from_View_alltickets);


    json_response = eval('(' + sessionStorage.getItem('ticketdetails_view') + ')');
    this.ticketdetailsresponse = json_response;

    if (sessionStorage.getItem('session_deptid') == "995" && sessionStorage.getItem("common_for_flats") != "view_my_tickets" || sessionStorage.getItem('session_deptid') == "994" && sessionStorage.getItem("common_for_flats") != "view_my_tickets") {
    ...
    } else if (sessionStorage.getItem("common_for_flats") == "view_my_tickets") {
  ...
  departmentList() {
    ...
    this.http.post(url, body, options).map(res => res.json()).subscribe(resp => {
      ...
      if (resp.responseCode == 200) {
        ...
        for (var i = 0; i < resp.ticketForwardMenuList.length; i++) {
          $('#sel').append("<option value='" + resp.ticketForwardMenuList[i].typeOf + "," + resp.ticketForwardMenuList[i].genericId + "'>" + resp.ticketForwardMenuList[i].item + "</option>");
        }
      } else if (resp.responseCode == 440) {
        swal("Your Session has been Timed Out!", "Please login once again.", "error");
        this.router.navigate([""]);
  ...
  getTicketDetails() {
    ...
    this.http.post(url, body, options).map(res => res.json()).subscribe(resp => {
      ...
      if (resp.responseCode == 200) {
        ...
        // alert(this.view_ticketdetails_status)

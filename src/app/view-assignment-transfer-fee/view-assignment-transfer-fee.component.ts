...
import { ViewAssignmentTransferFeeService } from './view-assignment-transfer-fee.service';
...
@Component({
  selector: 'app-view-assignment-transfer-fee',
  templateUrl: './view-assignment-transfer-fee.component.html',
  styleUrls: ['./view-assignment-transfer-fee.component.sass']
})
export class ViewAssignmentTransferFeeComponent implements OnInit {
  ...
  constructor(private router: Router, private service: ViewAssignmentTransferFeeService, private http: Http, public cmn: CommonComponent) {
  ...
  ngOnInit() {
    ...
    if (this.Approvependinginvoicedata !== null || this.Approvependinginvoicedata !== "null" || this.Approvependinginvoicedata !== "undefined" || this.Approvependinginvoicedata !== undefined) {
      ...
      this.modification_Invoice(this.Approvependinginvoicedata.siteId, this.Approvependinginvoicedata.bookingFormId, this.Approvependinginvoicedata.finBookingFormAssignmentTransferFeeId);
    ...
    this.service.getModificationInvoice(siteId, bookingFormId, finBookingFormModiCostId).then(resp => {
      ...
      if (resp.responseCode == 200) {
        ...
        if (resp.responseObjList.length !== 0) {
          ...
          this.modificationdetails = resp.responseObjList[0].finBookingFormAssignmentTransferFeeDtlsList;
  ...
  homeClick() {
    this.router.navigateByUrl("approve-AssignmentTransferFee-invoices");
  }
  ...
  Approvefun(usertype) {
    ...
    for (var i = 0; i < this.controller[0].finBookingFormAssignmentTransferFeeDtlsList.length; i++) {
      this.percentageId = this.controller[0].finBookingFormAssignmentTransferFeeDtlsList[i].percentageId;
      this.percentageValue = this.controller[0].finBookingFormAssignmentTransferFeeDtlsList[i].percentageValue;
      this.modiCostDtlsRequests.push({
        'finBookingAssignmentTransferFeeDetailsId': this.controller[0].finBookingFormAssignmentTransferFeeDtlsList[i].finBookingAssignmentTransferFeeDetailsId,
        'assignmentTransferFeeChargeDesc': this.controller[0].finBookingFormAssignmentTransferFeeDtlsList[i].assignmentTransferFeeChargeDesc,
        'units': this.controller[0].finBookingFormAssignmentTransferFeeDtlsList[i].units,
        'quantity': this.controller[0].finBookingFormAssignmentTransferFeeDtlsList[i].quantity,
        'rate': this.controller[0].finBookingFormAssignmentTransferFeeDtlsList[i].rate,
        'basicAmount': this.controller[0].finBookingFormAssignmentTransferFeeDtlsList[i].basicAmount,
        'actionType': 'N/A'
      });
    }
    ...
    $(".page-loader-wrapper").show();
    var url = this.cmn.commonUrl + "financial/approveAssignmentTransferFeeEntry.spring";
    ...
    var body = {
      ...
      "finBookingFormAssignmentTransferFeeId": this.controller[0].finBookingFormAssignmentTransferFeeId,
      ...
      "assignmentTransferFeeDtlsRequests": this.modiCostDtlsRequests,
    ...
    this.http.post(url, body, options).map(res => res.json()).subscribe(resp => {
      ...
      if (resp.responseCode == 200) {
        ...
        if (resp.responseObjList == null) {
          swal("Assignment Transfer Fee Invoice " + this.buttontype + " successfully");
          setTimeout(() => {
            this.router.navigateByUrl('approve-AssignmentTransferFee-invoices');
          }, 2000);
  ...
  siteBank_Ac_List(siteid, binval) {
    ...
    var body = {
      "sessionKey": "" + sessionStorage.getItem("login_sessionkey"),
      "siteIds": [siteid],
      "condition": "ASSIGNMENT_TRANSFER_FEE",

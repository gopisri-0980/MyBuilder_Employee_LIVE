...
@Component({
  selector: 'app-complaint-monthly-report-projectwise',
  templateUrl: './complaint-monthly-report-projectwise.component.html',
  styleUrls: ['./complaint-monthly-report-projectwise.component.sass']
})
export class ComplaintMonthlyReportProjectwiseComponent implements OnInit {
  ...
  completedTransactions_default(action) {
    ...
    let url = this.cmn.commonUrl + "complaintreport/getAllMainComplaintTypesProjectWiseComplaintCount.spring"
    ...
    this.http.post(url, body, options).map(res => res.json()).subscribe(resp => {
      ...
      if (resp.responseCode == 200) {
        this.controller_data.push("Complaint Type");
        ...
        for (var i = 0; i < resp.complaintReportingResponces[0].ticktTypewiseComplaintCount[0].siteList.length; i++) {
          this.controller_data.push(resp.complaintReportingResponces[0].ticktTypewiseComplaintCount[0].siteList[i].siteName);
          this.Total_controller.push(resp.complaintReportingResponces[0].ticktTypewiseComplaintCount[0].siteList[i].sitewisetotalComplaints);
        }
        this.controller_data.push("Grand Total");
        totaltransactionAmount = 0;

        for (var i = 0; i < resp.complaintReportingResponces[0].ticktTypewiseComplaintCount.length; i++) {
          console.log(resp.complaintReportingResponces[0].ticktTypewiseComplaintCount[i].totalComplaints);
          totaltransactionAmount += parseFloat(resp.complaintReportingResponces[0].ticktTypewiseComplaintCount[i].totalComplaints);
        }
        ...
        this.controller = resp.complaintReportingResponces[0].ticktTypewiseComplaintCount;
        setTimeout(function () {
